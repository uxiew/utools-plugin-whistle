...
/**
 * @description 执行 Whistle 命令
 * @param {string} cmd 命令字符
 */
async function runWhistleCmd(cmd: string) {
  try {
...
async function w2BtnRenew() {
  let w2_pre_running = w2Store.running;
  setpsStore.w2.loading = true;
  w2Store.running = false;
  await installWhistle();
  w2_pre_running && (await runWhistleCmd('restart'));
}
</script>
